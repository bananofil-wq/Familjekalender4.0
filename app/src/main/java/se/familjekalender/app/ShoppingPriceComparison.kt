...
data class ShoppingPriceResult(
    ...
    val cheapest: ShoppingPriceMatch?
        get() = matches.minByOrNull { it.price }
}
...
data class StoreBasketSummary(
    ...
    val complete: Boolean
        get() = totalItems > 0 && matchedItems == totalItems
}
...
data class ShoppingPriceComparison(
    ...
    val pricedItems: List<ShoppingPriceResult>
        get() = items.filter { it.cheapest != null }

    val splitBasketTotal: Double?
        get() =
            items
                .takeIf { it.isNotEmpty() && it.all { item -> item.cheapest != null } }
        ...
        return stores
            .map { store ->
                val selected = items.mapNotNull { item ->
                    item.matches.filter { it.store == store }.minByOrNull { it.price }
                }
                StoreBasketSummary(
                    store = store,
    ...
    val bestCompleteStore: StoreBasketSummary?
        get() = storeSummaries().filter { it.complete }.minByOrNull { it.total }

    val splitSavingsAgainstBestCompleteStore: Double?
        get() {
...
object ShoppingPriceService {
    private const val ENDPOINT =
        "https://zigychfkpgypjuovgyqq.supabase.co/functions/v1/compare-prices"
    private val excludedStores = setOf("jula", "rusta")

    private fun isExcludedStore(store: String): Boolean = store.trim().lowercase() in excludedStores

    suspend fun compare(names: List<String>): ShoppingPriceComparison =
                ...
                repeat(itemsArray.length()) { index ->
                            ...
                            if (!isExcludedStore(store)) {
                                add(
                                    ShoppingPriceMatch(
                                        ...
                                        brand =
                                            row.optString("brand").takeIf {
                                                it.isNotBlank() && it != "null"
                                            },
                                        packageText =
                                            row.optString("package").takeIf {
                                                it.isNotBlank() && it != "null"
                                            },
                                        price = row.optDouble("effectivePrice"),
                                        regularPrice =
                                            row.optDouble("regularPrice").takeIf { !it.isNaN() },
                                        memberPrice =
                                            row.optDouble("memberPrice").takeIf { !it.isNaN() },
                                        comparisonPrice =
                                            row.optDouble("comparisonPrice").takeIf { !it.isNaN() },
                                        comparisonUnit =
                                            row.optString("comparisonUnit").takeIf {
                                                it.isNotBlank() && it != "null"
                                            },
                                        confirmedAt =
                                            row.optString("confirmedAt").takeIf {
                                                it.isNotBlank() && it != "null"
                                            },
                                        sourceUrl =
                                            row.optString("sourceUrl").takeIf {
                                                it.isNotBlank() && it != "null"
                                            },
                                    )
                                )
                    ...
                    add(
                        ShoppingPriceResult(
                            ...
                            matches = matches,
                            error =
                                item.optString("error").takeIf { it.isNotBlank() && it != "null" },
                        )
                    )
                }
            }
