...
class RunRecordingService : Service() {
    companion object {
        ...
        fun finish(context: Context) {
            val intent =
                Intent(context, RunRecordingService::class.java).apply { action = ACTION_STOP }
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O)
                ContextCompat.startForegroundService(context, intent)
            else context.startService(intent)
        }
                ...
                for (i in 0 until array.length()) {
                    ...
                    add(
                        RecordedRoutePoint(
                            ...
                            accuracyMeters =
                                p.optDouble("accuracy", Double.NaN)
                                    .takeUnless { it.isNaN() }
                                    ?.toFloat(),
                            altitudeMeters =
                                p.optDouble("altitude", Double.NaN).takeUnless { it.isNaN() },
                        )
                    )
                }
            }
    ...
    private fun updateNotification(force: Boolean = false) {
        ...
        val text =
            "%.2f km · %02d:%02d:%02d"
                .format(distanceKm, elapsed / 3600, (elapsed % 3600) / 60, elapsed % 60)
            ...
            (getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager)
                .createNotificationChannel(
                    NotificationChannel(
                        ...
                        .apply {
                            description =
                                "Visas medan Familjekalendern spelar in en löprunda med GPS."
                        }
                )
        }
    }
