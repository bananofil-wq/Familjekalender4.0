...
private const val LOCATION_SUPABASE_ANON_KEY =
...
object FamilyLocationSync {
    suspend fun loadLocations(session: FamilySession): List<SyncFamilyLocation> =
                ...
                repeat(array.length()) {
                    ...
                    add(
                        SyncFamilyLocation(
                            ...
                            accuracyM =
                                if (row.isNull("accuracy_m")) null
                                else row.getDouble("accuracy_m").toFloat(),
                            batteryPercent =
                                if (row.isNull("battery_percent")) null
                                else row.getInt("battery_percent"),
    ...
    suspend fun stopSharing(session: FamilySession, memberId: String) =
        withContext(Dispatchers.IO) {
            request(
                "PATCH",
                "/rest/v1/family_locations?family_id=eq.${session.id}&member_id=eq.$memberId",
                JSONObject()
                    .put("sharing_enabled", false)
                    .put("updated_at", OffsetDateTime.now().toString()),
                session.code,
                preferRepresentation = false,
            )
    ...
    suspend fun loadPlaces(session: FamilySession): List<SyncFamilyPlace> =
        withContext(Dispatchers.IO) {
            val text =
                request(
                    "GET",
    ...
    suspend fun updatePlaceAlerts(
        ...
        withContext(Dispatchers.IO) {
            ...
            request(
                "PATCH",
                "/rest/v1/family_places?id=eq.${place.id}&family_id=eq.${session.id}",
                body,
                session.code,
                preferRepresentation = false,
            )
    ...
    suspend fun deletePlace(session: FamilySession, placeId: String) =
        withContext(Dispatchers.IO) {
            request(
                "DELETE",
                "/rest/v1/family_places?id=eq.$placeId&family_id=eq.${session.id}",
                familyCode = session.code,
                preferRepresentation = false,
            )
        }
