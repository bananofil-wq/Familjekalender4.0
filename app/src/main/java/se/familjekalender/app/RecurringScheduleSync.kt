...
private const val RECURRING_SUPABASE_ANON_KEY =
...
object RecurringScheduleSync {
    suspend fun loadTemplates(session: FamilySession): List<ScheduleTemplate> =
                ...
                repeat(array.length()) {
                    ...
                    add(
                        ScheduleTemplate(
                            ...
                            endTime =
                                if (row.isNull("end_time")) null
                                else row.getString("end_time").take(5),
                            memberId =
    ...
    suspend fun replaceTemplateFromWeek(
        ...
        withContext(Dispatchers.IO) {
            request(
                "DELETE",
                "/rest/v1/family_schedule_templates?family_id=eq.${session.id}&name=eq.${
                    encode(
                        templateName
                    )
                }",
            ...
            grouped.forEach { (_, rows) ->
                ...
                if (!first.endTime.isNullOrBlank()) body.put("end_time", first.endTime)
                else body.put("end_time", JSONObject.NULL)
                if (first.memberId == null || first.memberId == ALL_FAMILY_MEMBER_ID)
                    body.put("member_id", JSONObject.NULL)
                else body.put("member_id", first.memberId)
                request("POST", "/rest/v1/family_schedule_templates", session.code, body)
            }
            grouped.size
        }

    private fun pauseMatches(
        pause: SchedulePause,
        date: LocalDate,
        ...
        val titleMatches =
            pause.titlePrefix.isNullOrBlank() ||
                    template.title.startsWith(pause.titlePrefix, ignoreCase = true)
                ...
                if (duplicate) {
                    ...
                    val seriesId =
                        java.util.UUID.nameUUIDFromBytes(
                            "${session.id}|template|${template.id}"
    ...
    suspend fun loadPauses(session: FamilySession): List<SchedulePause> =
        withContext(Dispatchers.IO) {
            val result =
                request(
                    "GET",
                ...
                repeat(array.length()) {
                    ...
                    add(
                        SchedulePause(
                            ...
                            memberId =
                                if (row.isNull("member_id")) null else row.getString("member_id"),
                            titlePrefix =
                                if (row.isNull("title_prefix")) null
                                else row.getString("title_prefix"),
                        )
                    )
                }
            }
    ...
    private fun pauseMatchesEvent(pause: SchedulePause, event: SyncEvent): Boolean {
        ...
        if (pause.scope == "all_schedules") return true
        if (pause.scope == "series" && !pause.seriesId.isNullOrBlank())
            return pause.seriesId == event.seriesId
        val memberMatches = pause.memberId == null || pause.memberId == event.memberId
        val titleMatches =
            pause.titlePrefix.isNullOrBlank() ||
                    event.title.startsWith(pause.titlePrefix, ignoreCase = true)
        return memberMatches && titleMatches
    }
    ...
    suspend fun addGlobalPause(session: FamilySession, startsOn: LocalDate, endsOn: LocalDate) =
        withContext(Dispatchers.IO) {
    ...
    suspend fun deletePause(session: FamilySession, pauseId: String) =
        withContext(Dispatchers.IO) {
            request(
                "DELETE",
