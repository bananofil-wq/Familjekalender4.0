...
private const val EVENT_SUPABASE_ANON_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJIUzI1NiIsInJlZiI6InppZ3ljaGZrcGd5cGp1b3ZneXFxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODg2NTI2NzQsImV4cCI6MjEwNDIyODY3NH0.dN4zZ78EDYjPOpQ4-nj21tnFOJG21Hj7dXpm69AuEQc"
private val EVENT_STOCKHOLM = ZoneId.of("Europe/Stockholm")

/**
 * Full event writer used by the activity dialog. Unlike the old compatibility
 * shim, this persists both starts_at and ends_at so planning/conflict logic can
 * use the actual duration instead of silently throwing the selected end time away.
 */
suspend fun SupabaseSync.addEvent(
    ...
    val connection = URL("$EVENT_SUPABASE_URL/rest/v1/calendar_events").openConnection() as HttpURLConnection
