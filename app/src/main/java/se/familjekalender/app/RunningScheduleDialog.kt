...
private fun parseRunningEntry(event: SyncEvent): RunningEntry? {
    ...
    val distanceMatch =
        Regex("([0-9]+(?:[.,][0-9]+)?)\\s*km", RegexOption.IGNORE_CASE).find(event.title)
    val durationMatch = Regex("([0-9]+)\\s*min", RegexOption.IGNORE_CASE).find(event.title)
    val distance =
        distanceMatch?.groupValues?.getOrNull(1)?.replace(',', '.')?.toDoubleOrNull() ?: return null
            ...
            Column(
                ...
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    Button(
                        ...
                        colors =
                            if (!completedMode) ButtonDefaults.buttonColors()
                ...
                Button(
                    onClick = {
                        ...
                        when {
                            ...
                            !Regex("^([01]\\d|2[0-3]):[0-5]\\d$").matches(timeText.trim()) ->
                                status = "Ange tiden som TT:MM"

                            distance == null || distance <= 0.0 -> status = "Ange en giltig distans"
                            duration == null || duration <= 0 ->
                                status = "Ange en giltig tid i minuter"
                ...
                if (completedEntries.isEmpty()) {
                    ...
                    Text(
                        "Senast: ${formatDistance(latest.distanceKm)} km · ${latest.durationMinutes} min · ${
                            formatPace(
                                latest.paceMinutesPerKm
                            )
                        }",
                        fontSize = 12.sp,
                    )
                    if (best != null && completedEntries.size > 1) {
                        Text(
                            "Bästa tempo: ${formatPace(best.paceMinutesPerKm)}",
                            color = Muted,
                            fontSize = 12.sp,
                        )
                    }
                ...
                val upcoming =
                    runningEntries
                        .filter { !it.completed && !it.event.date.isBefore(LocalDate.now()) }
                        .take(5)
